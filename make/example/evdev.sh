...
defines+=("-DPUNKNOBS_EXAMPLE_EVDEV")

case $poll in
	poll)
	defines+=("-DPUNKNOBS_EXAMPLE_EVDEV_POLL")
	;;
	epoll)
	defines+=("-DPUNKNOBS_EXAMPLE_EVDEV_EPOLL")
	;;
	*)
...
#defines+=("-DPUNKNOBS_EXAMPLE_LOG_ALL")
...
ninja_file=example_evdev.ninja
src+=("example/example.c")
ldlibs+=("-levdev")
ldlibs+=("-lpthread")

# additional object files
libs+=("\$folder_library/evdev/$name_lib.a")
libs+=("\$folder_library/punknobs_elf.a")
...
{ \
echo "rule generator"; \
echo "    command = make/example/evdev.sh $build $poll"; \
