...
src+=("src/common/punknobs.c")
src+=("src/common/punknobs_error.c")

# save symbols file path
symbols_file="src/common/symbols_common.txt"
...
{ \
echo "# copy headers"; \
echo "build \$folder_include/punknobs.h: \$"; \
echo "cp src/include/punknobs.h"; \
echo ""; \
} >> "$output/$ninja_file"

{ \
echo "build headers: phony \$"; \
echo "\$folder_include/punknobs.h"; \
echo ""; \
} >> "$output/$ninja_file"
